...
class kalman_filter:
    ...
    def __init__(self, P,Q,R, x, dt):
        ...
        self.P = P      # Covariance of the estimate
        self.Q = Q      # Covariance of the process
        self.R = R      # Covariance of the measurements
        self.x = x      # Initial state [x, y, th, w, v, vdot]
    ...
    def motion_model(self):
        ...
        # Compute the new state in the stationary frame
        self.x = np.array([
            x + v * np.cos(th) * dt,
            y + v * np.sin(th) * dt,
            th + w * dt,
    ...
    def jacobian_A(self):
        ...
        # Compute the jacobian of the motion model (derivative of the motion model wrt each state)
        return np.array([
            #x, y, th,                   w,  v,               vdot
    ...
    def jacobian_H(self):
        ...
        # Compute the jacobian of the measurement model (derivative of the measurement model wrt each state)
        return np.array([
            #x, y,th, w, v,vdot
            [0, 0, 0, 0, 1, 0],  # v
            [0, 0, 0, 1, 0, 0],  # w
